...
class ControlMaker(tk.Frame):
    def __init__(self, section_name, master=None, **options):
        ...
        self.l_header.grid(sticky=tk.W, row=0, column=0, columnspan=3, padx=cfg.xd, pady=cfg.yd)
        self.ls = []
        row = 1
        ...
        for par in self.mctrl.section_par_dict[self.sn].keys():
            ...
            self.par_objects[par].grid(sticky=tk.E, row=row, column=1, padx=cfg.xd, pady=cfg.yd)
            row += 1
    ...
    def add(self):
        ...
        if self.sn == "stab":
            ...
            tk.Button(self, text="Estimate", command=lambda: self.pop_time()).grid(sticky=tk.W, row=self.par_rows["Timestep"], column=2, padx=cfg.xd, pady=cfg.yd)
        if self.sn == "out":
            self.map_formats = []
            self.map_data_types = {}
            tk.Button(self, text="Add", command=lambda: self.format_add()).grid(sticky=tk.W, row=self.par_rows["Map Output Format"], column=2, padx=cfg.xd, pady=cfg.yd)
            tk.Button(self, text="Reset formats", command=lambda: self.format_clear(), width=10).grid(sticky=tk.W, row=self.par_rows["Map Output Format"], column=3, padx=cfg.xd, pady=cfg.yd)
            tk.Label(self, text=" for FORMAT: ").grid(sticky=tk.W, row=self.par_rows["Map Output Data Types"], column=2, padx=cfg.xd, pady=cfg.yd)
            self.cbx_selected_formats = ttk.Combobox(self, width=10)
            self.cbx_selected_formats.grid(sticky=tk.E, row=self.par_rows["Map Output Data Types"], column=3, padx=cfg.xd, pady=cfg.yd)
            self.cbx_selected_formats['state'] = 'readonly'
            tk.Button(self, text="Add", command=lambda: self.format_data_type_add()).grid(sticky=tk.W, row=self.par_rows["Map Output Data Types"], column=4, padx=cfg.xd, pady=cfg.yd)
            tk.Button(self, text="Reset", command=lambda: self.format_data_type_clear()).grid(sticky=tk.W, row=self.par_rows["Map Output Data Types"], column=5, padx=cfg.xd, pady=cfg.yd)
